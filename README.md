...
Lattice is a native Apple notes app for quickly writing Markdown notes into a
portable local folder. The macOS app launches as a regular Dock app, and the
iOS app target lives in the same Xcode workspace with the shared Swift note
storage core.
...
Open the Apple workspace in Xcode:

```bash
open apps/Lattice/Lattice.xcworkspace
```

Build the iOS app target for Simulator:
...
xcodebuild -resolvePackageDependencies \
  -workspace apps/Lattice/Lattice.xcworkspace \
  -scheme LatticeMac \
  -derivedDataPath .build/xcode

sparkle_bin="$(find .build/xcode/SourcePackages/artifacts -path '*/Sparkle/bin' -type d -print -quit)"
"$sparkle_bin/generate_keys" --account lattice-dev

LATTICE_SPARKLE_FEED_URL=http://localhost:8000/appcast.xml \
LATTICE_SPARKLE_PUBLIC_ED_KEY="$("$sparkle_bin/generate_keys" --account lattice-dev -p)" \
