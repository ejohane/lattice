...
final class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  private var notesFolderSetupWindowController: NotesFolderSetupWindowController?
  ...
  private func makeMenu() -> NSMenu {
    ...
    let notesFolderTitle: String
    if let url = noteStore.activeNotesFolderURL(),
       noteStore.validateNotesFolder(at: url).isUsable {
      notesFolderTitle = "Notes Folder: \(url.lastPathComponent)"
    } else {
      notesFolderTitle = "Notes Folder: Not Selected"
    }
    let notesFolderItem = NSMenuItem(
      title: notesFolderTitle,
      action: nil,
      keyEquivalent: ""
    )
    notesFolderItem.isEnabled = false
    menu.addItem(notesFolderItem)
    ...
    menu.addItem(NSMenuItem(
      ...
      action: #selector(revealCurrentNote),
      keyEquivalent: ""
    ))
    menu.addItem(NSMenuItem(
  ...
  @objc private func openNotesFolder() {
    guard let url = noteStore.activeNotesFolderURL(),
          noteStore.validateNotesFolder(at: url).isUsable else {
      showNotesFolderSetup()
      return
    }
  ...
  @objc private func revealCurrentNote() {
    guard let noteURL = noteStore.activeNoteURL(),
          FileManager.default.fileExists(atPath: noteURL.path) else {
      NSSound.beep()
      return
    }
  ...
  private func showMainWindow() {
    guard hasUsableActiveNotesFolder() else {
      showNotesFolderSetup()
      return
    }
  ...
  private func showNotesFolderSetup() {
    if notesFolderSetupWindowController == nil {
      notesFolderSetupWindowController = NotesFolderSetupWindowController(
        noteStore: noteStore,
        onNotesFolderSelected: { [weak self] in
          self?.notesFolderSetupWindowController?.close()
          self?.notesFolderSetupWindowController = nil
          self?.refreshMenu()
          self?.showMainWindow()
        }
      )
    ...
    notesFolderSetupWindowController?.showWindow(nil)
    notesFolderSetupWindowController?.window?.makeKeyAndOrderFront(nil)
    NSApp.activate(ignoringOtherApps: true)
    refreshMenu()
  }

  private func hasUsableActiveNotesFolder() -> Bool {
    guard let url = noteStore.activeNotesFolderURL() else {
      return false
    }

    return noteStore.validateNotesFolder(at: url).isUsable
  }
...
extension MainWindowController: NSToolbarDelegate {
  ...
  private func presentSaveError(_ error: Error) {
    let alert = NSAlert()
    alert.messageText = "Note not saved"
...
@MainActor
final class NotesFolderSetupWindowController: NSWindowController {
  private let noteStore: NoteStore
  private let onNotesFolderSelected: () -> Void
  private let defaultPathLabel = NSTextField(labelWithString: "")

  init(
    noteStore: NoteStore,
    onNotesFolderSelected: @escaping () -> Void
  ) {
    self.noteStore = noteStore
    self.onNotesFolderSelected = onNotesFolderSelected
    ...
    window.title = "Choose Notes Folder"
    window.isReleasedWhenClosed = false
    window.center()
  ...
  override func showWindow(_ sender: Any?) {
    defaultPathLabel.stringValue = noteStore.defaultNotesFolderURL.path
    super.showWindow(sender)
  }
  ...
  private func buildContent() {
    ...
    let titleLabel = NSTextField(labelWithString: "Choose a notes folder")
    titleLabel.font = .systemFont(ofSize: 22, weight: .semibold)

    let descriptionLabel = NSTextField(wrappingLabelWithString: "Lattice stores portable Markdown notes in a local folder. Choose an existing folder or initialize the default location.")
    descriptionLabel.font = .systemFont(ofSize: 13, weight: .regular)
    descriptionLabel.textColor = .secondaryLabelColor
    ...
    let useDefaultButton = NSButton(
      ...
      target: self,
      action: #selector(useDefaultNotesFolder)
    )
  ...
  @objc private func useDefaultNotesFolder() {
    handleSelectedNotesFolder(noteStore.defaultNotesFolderURL)
  }
  ...
  @objc private func chooseFolder() {
    ...
    handleSelectedNotesFolder(url)
  }

  private func handleSelectedNotesFolder(_ url: URL) {
    switch noteStore.validateNotesFolder(at: url) {
    case .valid:
      select(url)
    case .uninitialized:
  ...
  private func confirmInitialization(at url: URL) -> Bool {
    let alert = NSAlert()
    alert.messageText = "Initialize this folder for Lattice notes?"
    alert.informativeText = "Lattice will create a notes folder in:\n\n\(url.path)"
    alert.alertStyle = .informational
    alert.addButton(withTitle: "Initialize Notes Folder")
  ...
  private func presentError(_ message: String) {
    let alert = NSAlert()
    alert.messageText = "Notes folder unavailable"
