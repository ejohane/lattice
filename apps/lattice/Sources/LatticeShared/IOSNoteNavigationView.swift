...
private enum IOSNoteSource: Hashable {
  case allNotes
  case tag(String)
}

struct IOSNoteSourceSidebar: View {
      ...
      Section("Tags") {
        if model.tagSummaries.isEmpty {
          ...
          ForEach(model.tagSummaries) { tag in
            ...
            .tag(IOSNoteSource.tag(tag.normalizedName))
            .contextMenu {
              Button {
  ...
  private var sourceSelection: Binding<IOSNoteSource?> {
    Binding {
...
@MainActor
private struct IOSNoteListRow: View {
  let title: String
  let excerpt: String
  let modifiedAt: Date?
